...
- **Every package owns a root namespace, including the root package.** That is what makes `ModuleNamespace.parent: None` mean "no ancestor" and nothing else, so a plain `.parent` walk (`is_ancestor_or_self`) terminates inside the package it started in and can never cross into another one. It used to mean *both* "no ancestor" and "the root package's own top-level scope" (which had no `tir.namespaces` entry, being backed by a global symbol table instead) — so a lookup walking past any package boundary fell through into the root's items, and `is_ancestor_or_self` needed a hand-written stop at package roots to compensate. Don't reintroduce a global fallback in `lookup_global_symbol`: running out of parents *is* the package boundary. A package has no name of its own either — it's known by the `dependencies` key whoever declared it used, so use `TIR::namespace_name(ns, packages, from)` rather than looking for a name on the namespace.
