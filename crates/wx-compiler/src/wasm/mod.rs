//! The WASM-shaped instruction representation every lowering pass targets —
//! a flat, linear sequence of instructions for a stack machine, as opposed
//! to the tree (`mir::Function`) or graph (`opt::Function`) representations
//! earlier in the pipeline. Two independent processes build a [`Function`]:
//! `opt::scheduler` (from the sea-of-nodes graph) and, eventually,
//! `mir::scheduler` (directly from MIR). Neither owns these types — they're
//! the shared contract both target and `codegen` consumes — which is also
//! why the few optimization passes that operate purely on this
//! representation ([`coalesce_locals`], [`peephole_local_tee`]) live here
//! rather than inside either producer.
...
/// the `ScalarType`-producing counterpart to `mir::flatten_type` (which
/// stops at MIR leaf types). The one place this conversion happens; every
/// producer of a `Function` should call this rather than repeating the
/// `mir::flatten_type` + `ScalarType::try_from` pair itself.
pub fn flatten_type_to_scalars(
	ty: mir::Type,
	aggregates: &[mir::Aggregate],
) -> Vec<ScalarType> {
	mir::flatten_type(ty, aggregates)
		.into_iter()
		.map(|t| ScalarType::try_from(t).expect("must be scalar"))
		.collect()
}
...
/// Debug-only metadata for one source-declared local, resolved to the wasm
/// local slot(s) it actually occupies — the wasm-scheduling-side
/// counterpart to `mir::LocalDebugInfo` (name and, for a struct-typed
/// local, its field names), plus the `wasm_local_start`/`wasm_local_count`
/// range only scheduling knows. Always empty for `opt::scheduler`'s output,
/// same as `Function::spans` — see there for why.
#[cfg_attr(test, derive(serde::Serialize))]
#[derive(Clone)]
pub struct LocalDebugInfo {
	pub name: SymbolU32,
	/// The local's MIR type — flattening it (`mir::flatten_type`) recovers
	/// each leaf's byte size for `DW_OP_piece` chains, and it's also what a
	/// type-DIE builder walks for struct member types. Not derivable from
	/// `wasm_local_count` alone (that's just a scalar count, not enough to
	/// reconstruct e.g. a struct's field types).
	pub ty: mir::Type,
	pub struct_debug: Option<mir::StructDebugInfo>,
	pub wasm_local_start: u32,
	pub wasm_local_count: u32,
}
...
pub struct Function {
	...
	/// Source span for `body[i]`, one-to-one with `body` — empty for
	/// `opt::scheduler`'s output, since CSE/scheduling there means an
	/// instruction no longer corresponds to a single source position.
	/// Populated by `mir::scheduler`, whose no-reordering guarantee is
	/// exactly what makes a per-instruction span meaningful.
	pub spans: Vec<ast::TextSpan>,
	/// One entry per named local actually emitted — empty for
	/// `opt::scheduler`'s output; see [`LocalDebugInfo`].
	pub locals_debug: Vec<LocalDebugInfo>,
}

/// A subset of WASM instructions produced by a scheduling pass.
...
pub enum Instruction {
	...
	CallIndirectSym {
		signature_index: mir::SignatureIndex,
	},
	// Memory
