...
/// `(path relative to `std/`, source)` and sorted by path. Generated by
/// `build.rs` — see there for why this is a directory walk rather than a
/// hand-maintained list.
///
/// The paths are exactly the keys the module resolver looks up, so
/// `std/math/mod.wx` appears here as `math/mod.wx`, which is what a
/// `module math;` declaration in `main.wx` resolves to.
pub const STDLIB_FILES: &[(&str, &str)] =
	include!(concat!(env!("OUT_DIR"), "/stdlib_files.rs"));

/// The stdlib crate root, relative to `std/` — where `load_stdlib` starts.
pub const STDLIB_ENTRY: &str = "main.wx";

/// The source of one embedded stdlib file, keyed as in [`STDLIB_FILES`].
///
/// Used to serve `wx://std/...` documents to the editor, so that
/// go-to-definition can land inside the stdlib without it existing on disk.
pub fn stdlib_file(path: &str) -> Option<&'static str> {
	STDLIB_FILES
		.iter()
...
impl CompilationUnitBuilder {
	...
	/// Loads the stdlib as a named `std` crate from its embedded sources.
	///
	/// Every file in [`STDLIB_FILES`] is handed to the loader up front, so the
	/// stdlib is an ordinary multi-file crate from here on: `module foo;` in
	/// `main.wx` resolves against this map exactly as it would against the
	/// filesystem for a user crate. Adding a stdlib file needs no change here.
	///
	/// The entry point is embedded, so this can never actually hit the "entry
	/// point unreadable" case `load_crate` guards against.
	pub fn load_stdlib(&mut self) -> CrateId {
		let files: HashMap<String, String> = STDLIB_FILES
			.iter()
			.map(|(path, source)| (path.to_string(), source.to_string()))
			.collect();
		self.load_library(
			"std",
			STDLIB_ENTRY.to_string(),
	...
	pub fn build(self, root_crate: CrateId) -> CompilationUnit {
		CompilationUnit {
			files: self.files,
			crates: self.crates,
			root_crate,
