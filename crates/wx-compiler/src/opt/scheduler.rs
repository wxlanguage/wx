...
//! # Output
//!
//! The scheduler produces a [`WasmFunction`] containing
//! - the extra WASM locals needed for spilled nodes (appended after params)
//! - a flat `Vec<Instruction>` for the function body
//!
//! `WasmFunction`/`Instruction` and everything else describing that output
//! shape live in `crate::wasm` — shared with `mir::scheduler`, the other
//! producer of the same representation — not here. Encoding `Instruction`s
//! to bytes is left to the codegen layer.
...
use crate::opt::{
	...
	MemAccess, StackResult, SwitchCase,
};
use crate::wasm::{
	self, BlockType, Function as WasmFunction, Instruction, Local, MemArg,
	ScalarType,
};
...
impl<'f> Scheduler<'f> {
	pub fn schedule(func: &'f Function, mir: &'f mir::MIR) -> WasmFunction {
	...
	fn emit_switch_br_table(
		...
		let len = depths.len() as u32;
		self.br_table_depths.extend(depths);
		self.body.push(Instruction::BrTable { start, len });
